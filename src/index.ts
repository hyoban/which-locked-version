...
export function parseLockFile(lockfile: string): PackageVersionMap {
  const json = isJson(lockfile)
  if (!json) {
    if (
      lockfile.startsWith(`# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
# yarn lockfile v1`)
    ) {
      return parseYarnLockFileV1(lockfile)
    }

    return parsePnpmLockFile(parse(lockfile))
  }
  return parseNpmLockFile(json)
}

function isJson(text: string): false | object {
  try {
